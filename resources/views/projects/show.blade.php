        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-lg text-neutral-600">{{ __('Project Details & Communication') }}</p>
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-soft border border-neutral-100 p-8">
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-2xl font-bold text-neutral-900">{{ __('Project Information') }}</h2>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <h3 class="text-lg font-semibold text-neutral-900 mb-2">{{ __('Service') }}</h3>
                            <p class="text-primary-600 font-medium">{{ $project->service->name ?? 'Not specified' }}</p>
                        </div>
                        <div>
                            <h3 class="text-lg font-semibold text-neutral-900 mb-2">{{ __('Budget') }}</h3>
                            <p class="text-2xl font-bold text-primary-600">${{ number_format($project->budget, 2) }}</p>
                        </div>
                        <div>
                            <h3 class="text-lg font-semibold text-neutral-900 mb-2">{{ __('Start Date') }}</h3>
                            <p class="text-neutral-700">{{ $project->start_date->format('M d, Y') }}</p>
                        </div>
                        <div>
                            <h3 class="text-lg font-semibold text-neutral-900 mb-2">{{ __('End Date') }}</h3>
                            <p class="text-neutral-700">{{ $project->end_date->format('M d, Y') }}</p>
                        </div>
                    </div>

                    <div class="mt-6">
                        <h3 class="text-lg font-semibold text-neutral-900 mb-2">{{ __('Description') }}</h3>
                        <p class="text-neutral-700 leading-relaxed">{{ $project->description }}</p>
                    </div>

                    <div class="mt-6">
                        <h3 class="text-lg font-semibold text-neutral-900 mb-2">{{ __('Requirements') }}</h3>
                ...
                <!-- Messages Section -->
                <div class="bg-white rounded-2xl shadow-soft border border-neutral-100 p-8">
                    <h2 class="text-2xl font-bold text-neutral-900 mb-6">{{ __('Project Messages') }}</h2>
                    ...
                    @else
                        <div class="text-center py-8">
                            ...
                            <p class="text-neutral-600">{{ __('No messages yet. Start the conversation!') }}</p>
                        </div>
                    @endif
                    ...
                    <form action="{{ route('messages.store') }}" method="POST" class="mt-6">
                        ...
                        <div class="flex space-x-3">
                            <textarea name="content" rows="3" class="flex-1 border border-neutral-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent" placeholder="{{ __('Type your message...') }}" required></textarea>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-soft border border-neutral-100 p-6">
                    <h3 class="text-lg font-semibold text-neutral-900 mb-4">{{ __('Project Stats') }}</h3>
                    <div class="space-y-3">
                        <div class="flex items-center justify-between">
                            <span class="text-neutral-600">{{ __('Created') }}</span>
                            <span class="font-medium">{{ $project->created_at->format('M d, Y') }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-neutral-600">{{ __('Messages') }}</span>
                            <span class="font-medium">{{ $project->messages->count() }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-neutral-600">{{ __('Days Remaining') }}</span>
                ...
                <!-- Quick Actions -->
                <div class="bg-white rounded-2xl shadow-soft border border-neutral-100 p-6">
                    <h3 class="text-lg font-semibold text-neutral-900 mb-4">{{ __('Quick Actions') }}</h3>
                    <div class="space-y-3">
                        <a href="{{ route('messages.index') }}" class="flex items-center space-x-3 p-3 bg-neutral-50 rounded-lg hover:bg-neutral-100 transition-colors duration-200">
                            ...
                            <span class="text-sm font-medium">{{ __('View All Messages') }}</span>
                        </a>
                        <a href="/contact" class="flex items-center space-x-3 p-3 bg-neutral-50 rounded-lg hover:bg-neutral-100 transition-colors duration-200">
                            ...
                            <span class="text-sm font-medium">{{ __('Contact Support') }}</span>
                        </a>
                    </div>
                </div>
            </div>
